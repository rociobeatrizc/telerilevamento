...
install.packages("rgdal")     # Dati spaziali
install.packages("RStoolbox") # Processing di immagini telerilevate.
install.packages("rasterdiv") # Indici di diversità
...
# L'acqua assorbe molto nel NIR, infatti l'acqua tende al nero nella seconda immagine.
# Nell'immagine sopra è visibile il sedimento del fiume, simile alla colorazione del suolo nudo. 
...
plot(dvi1992, col=cl)
plot(dvi2006, col=cl)
# Giallo = molta deforestazione. Per distruggere un habitat basta frammentarlo a sufficienza .
# In questo modo infatti le popolazioni non vengono più a contatto. 

# Si possono usare i nomi per selezionare. 
...
# NDVI: differenza normalizzata. 
# Si divide la differenza per la somma delle due bande: in questo modo si possono fare confronti. 
